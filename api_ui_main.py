...
async def some(url, remote_path):
...
def add_stream(url, remote_path):
    asyncio.run(some(url, remote_path))
...
def main():
    ...
    if selected == "API запросы":
        ...
        # cюда апи
        st.button(label="Добавить", on_click=add_stream, args=(url, remote_path))
